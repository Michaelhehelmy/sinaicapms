...
`backend/src/middleware/rateLimit.js` is KV-backed (distributed) with an in-memory per-isolate fallback and **fail-closed** behavior on KV errors. The Cloudflare **free plan caps KV writes at 1,000/day** — a KV write per API request exhausts it and every API call returns `429 Rate limit check failed` (this happened in production 2026-08-03). The `RATE_LIMIT_KV_ENABLED` var (`"false"` in `backend/wrangler.toml`) forces the in-memory fallback (zero KV writes; `rateLimit.js` ~:145-177). Only set it to `"true"` on a plan with adequate KV quota. **Accepted trade-off (2026-09-16)**: the in-memory fallback is **per-isolate** — the counter `Map` lives on `globalThis` in the isolate's process, so limits are **per-isolate rather than global** (scaling isolates out = effectively higher limits), counters **reset on isolate restart**, and the limiter **cannot coordinate across isolates** (a distributed attacker can burst once per isolate). It still fails closed on exceptions (`429 Rate limit check failed`). Distributed KV limiting stays off because of the free-plan write quota; revisit by flipping `RATE_LIMIT_KV_ENABLED` to `"true"` only on a plan with enough quota. Follow-up hardening (per-account username lockout) is backlogged in `.opencode/agents/tmp/PLAN-BACKLOG.md`.
