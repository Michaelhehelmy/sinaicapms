...
describe('useSseInbox', () => {
  ...
  it('appends lastEventId to the stream URL', async () => {
    const { result } = renderHook(() => useSseInbox({ ...baseProps, lastEventId: 'evt_9' }));
    await flush();
  ...
  it('sends the latest received id as lastEventId on reconnect', async () => {
    ...
    act(() => {
      FakeEventSource.instances[0].onmessage?.({
        data: 'data: {"type":"new-lead","leadId":"l1"}',
        lastEventId: '17',
      });
    });
    act(() => {
    ...
    expect(FakeEventSource.instances).toHaveLength(2);
    expect(FakeEventSource.instances[1].url).toContain('lastEventId=17');
  });
  ...
  it('forwards onReset from the stream to the caller', async () => {
    const onReset = vi.fn();
    renderHook(() => useSseInbox({ ...baseProps, onReset }));
    await flush();
