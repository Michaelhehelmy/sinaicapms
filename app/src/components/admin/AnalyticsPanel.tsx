...
import type { KitchenTrend } from '@/lib/api';
...
function StackedBarChart({ items, labelKey, stacks }: { items: KitchenTrend[]; labelKey: string; stacks: { key: string; color: string; label: string }[] }) {
  if (!items.length) return <EmptyState title="No data" />;
  const asRecord = (item: KitchenTrend) => item as unknown as Record<string, unknown>;
  const maxTotal = Math.max(...items.map((item) => stacks.reduce((sum, s) => sum + (Number(asRecord(item)[s.key]) || 0), 0)));
  return (
    <div className="space-y-2">
      {items.map((item, idx) => {
        const rec = asRecord(item);
        const total = stacks.reduce((sum, s) => sum + (Number(rec[s.key]) || 0), 0);
        return (
          <div key={idx} className="flex items-center gap-3">
            <span className="text-xs text-gray-500 w-20 shrink-0">{String(rec[labelKey]).slice(5)}</span>
            <div className="flex-1 flex h-5 rounded-full overflow-hidden bg-gray-100">
              {stacks.map((s, si) => {
                const val = Number(rec[s.key]) || 0;
...
const KITCHEN_STATUS_LABEL: Record<string, { text: string; variant: 'success' | 'warning' | 'info' | 'danger' | 'neutral' }> = {
...
/* ── Per-tab components.  T22: each tab module mounts ONLY when active, so
   mounting the panel fires exactly the active tab's queries (was all 8 on
   mount behind one loading OR-gate). Each tab shows its own skeleton while
   its queries are in flight. ─────────────────────────────── */
...
function OverviewTab({ days }: { days: number }) {
  ...
  if (revLoading || occLoading || ksLoading || scLoading) return <LoadingSpinner text="Loading analytics..." />;

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Revenue" value={revenue ? formatCurrency(revenue.summary.totalRevenue) : '-'} sub={`${days} day period`} />
...
function KitchenTab({ days }: { days: number }) {
  const { data: kitchen, isLoading: ksLoading } = useKitchenPerformanceQuery(Math.min(days, 30));
  if (ksLoading) return <LoadingSpinner text="Loading analytics..." />;
...
function RevenueTab({ days }: { days: number }) {
  ...
  if (rbLoading) return <LoadingSpinner text="Loading analytics..." />;
...
function CustomersTab({ days }: { days: number }) {
  ...
  if (cmLoading) return <LoadingSpinner text="Loading analytics..." />;
...
export default function AnalyticsPanel() {
  ...
  // T22: no queries fire here — the active tab's subcomponent owns its queries,
  // so mounting the panel issues exactly one tab's request set.
  return (
    <div className="space-y-6" data-testid="analytics-panel">
      ...
      {tab === 'kitchen' && <KitchenTab days={days} />}
