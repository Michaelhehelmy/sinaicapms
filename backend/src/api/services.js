...
router.get('/definitions', async (c) => {
  ...
  return jsonResponse(rows.results.map(toCamel));
});
...
router.post('/definitions', async (c) => {
  ...
  try {
    ...
    return jsonResponse({ id, slug: finalSlug, success: true }, 201);
  } catch (e) {
    if (String(e).includes('UNIQUE')) return errorResponse('A definition with this slug already exists', 409);
    throw e;
  }
});
...
router.post('/items', async (c) => {
  ...
  if (!def) return errorResponse('Service definition not found', 404);
  const id = crypto.randomUUID();
  await c.env.DB.prepare(
...
router.post('/bookings', async (c) => {
  ...
  if (item.status !== 'active') return errorResponse('Service item is not available', 400);
  // Check for double-booking if scheduled_date is provided — atomic guard
  // via INSERT ... SELECT WHERE NOT EXISTS to prevent race conditions.
  const id = crypto.randomUUID();
  if (scheduled_date) {
    const insertResult = await c.env.DB.prepare(
      `INSERT INTO service_bookings (id, tenant_id, service_item_id, customer_name, customer_phone, scheduled_date, notes)
       ...
       WHERE NOT EXISTS (
         ...
         WHERE service_item_id = ? AND scheduled_date = ? AND status NOT IN ('canceled') AND tenant_id = ?
       )`
    ).bind(id, tenantId, service_item_id, customer_name || null, customer_phone || null, scheduled_date, notes || null,
           service_item_id, scheduled_date, tenantId).run();
    ...
    await c.env.DB.prepare(
      `INSERT INTO service_bookings (id, tenant_id, service_item_id, customer_name, customer_phone, scheduled_date, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    ).bind(id, tenantId, service_item_id, customer_name || null, customer_phone || null, null, notes || null).run();
  }
  return jsonResponse({ id, success: true, status: 'pending' }, 201);
});
...
router.patch('/bookings/:id/status', async (c) => {
  ...
  if (!validTransitions[booking.status]?.includes(status)) {
    return errorResponse(`Cannot transition from '${booking.status}' to '${status}'`, 400);
  }
  await c.env.DB.prepare(
    'UPDATE service_bookings SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND tenant_id = ?'
  ).bind(status, id, tenantId).run();
  return jsonResponse({ id, status, success: true });
});
...
router.patch('/bookings/:id/assign', async (c) => {
  ...
  if (!assigned_worker_id) return errorResponse('assigned_worker_id is required', 400);
  const booking = await c.env.DB.prepare(
  ...
  return jsonResponse({ id, assigned_worker_id, success: true });
});
...
router.post('/items/:id/availability', async (c) => {
  ...
  if (!available_date) return errorResponse('available_date is required', 400);
  ...
  return jsonResponse({ id: slotId, success: true }, 201);
});
...
router.delete('/availability/:id', async (c) => {
  ...
  if (!slot) return errorResponse('Availability slot not found', 404);
  await c.env.DB.prepare(
...
router.post('/reviews', async (c) => {
  ...
  if (!service_item_id) return errorResponse('service_item_id is required', 400);
  ...
  return jsonResponse({ id: reviewId, success: true }, 201);
});
...
router.put('/items/:id/pricing', async (c) => {
  ...
  if (!price_tier || !['standard', 'premium', 'luxury'].includes(price_tier)) {
    return errorResponse("price_tier must be 'standard', 'premium', or 'luxury'", 400);
  }
  const item = await c.env.DB.prepare(
    'SELECT id FROM service_items WHERE id = ? AND tenant_id = ?'
  ).bind(id, tenantId).first();
