...
export async function handleCampsRoute(request, env, tenantId) {
  ...
  if (method === 'GET') {
    if (path.length === 2) {
      ...
      const bindings = marketplace ? [path[2]] : [tenantId, path[2]];
      const { results } = await env.DB.prepare(query).bind(...bindings).all();
    ...
    try {
      const parsed = campPostSchema.safeParse(toSnake(await request.json()));
      if (!parsed.success) {
    ...
    try {
      const campId = path[2];
      const parsed = campPutSchema.safeParse(toSnake(await request.json()));
      if (!parsed.success) {
        return validationError(parsed);
      }
    ...
    try {
      const campId = path[2];
      // Verify ownership
      const { results: check } = await env.DB.prepare("SELECT id FROM camps WHERE tenant_id = ? AND id = ?").bind(tenantId, campId).all();
      ...
      await env.DB.prepare("DELETE FROM rooms_new WHERE camp_id = ?").bind(campId).run();
      // Cascade: delete product_camps associations
      await env.DB.prepare("DELETE FROM product_camps WHERE camp_id = ?").bind(campId).run();
      // Finally: delete the camp
      await env.DB.prepare("DELETE FROM camps WHERE tenant_id = ? AND id = ?").bind(tenantId, campId).run();
      return jsonResponse({ success: true });
    } catch (e) {
...
export async function handleProductsRoute(request, env, tenantId) {
  const url = new URL(request.url);
  const method = request.method;
  const path = url.pathname.split('/').filter(Boolean);
  ...
  if (method === 'GET') {
    ...
    try {
      const parsed = productPostSchema.safeParse(toSnake(await request.json()));
      if (!parsed.success) {
        return validationError(parsed);
      }
    ...
    try {
      const pid = path[2];
      const parsed = productPostSchema.partial().safeParse(toSnake(await request.json()));
      if (!parsed.success) {
        return validationError(parsed);
      }
      ...
      if (camp_ids && Array.isArray(camp_ids)) {
        await env.DB.prepare("DELETE FROM product_camps WHERE product_id = ?").bind(pid).run();
    ...
    try {
      ...
      const { results: usedRooms } = await env.DB.prepare(
        "SELECT r.id FROM rooms_new r JOIN camps c ON r.camp_id = c.id WHERE c.tenant_id = ? AND r.product_id = ?"
      ).bind(tenantId, pid).all();
      const { results: usedRates } = await env.DB.prepare(
      ...
      await env.DB.prepare("DELETE FROM product_camps WHERE product_id = ?").bind(pid).run();
      await env.DB.prepare("DELETE FROM pos_products WHERE tenant_id = ? AND id = ?").bind(tenantId, pid).run();
...
export async function handleRoomsRoute(request, env, tenantId) {
  const url = new URL(request.url);
  const method = request.method;
  const path = url.pathname.split('/').filter(Boolean);
  ...
  if (method === 'GET') {
    ...
    let query = "SELECT r.* FROM rooms_new r JOIN camps c ON r.camp_id = c.id WHERE c.tenant_id = ?";
    ...
    try {
      const parsed = roomPostSchema.safeParse(toSnake(await request.json()));
      if (!parsed.success) {
        return validationError(parsed);
      }
      ...
      if (finalMaxGuests === undefined || finalMaxGuests === null) {
        const { results: prod } = await env.DB.prepare("SELECT capacity FROM pos_products WHERE tenant_id = ? AND id = ?").bind(tenantId, product_id).all();
        finalMaxGuests = prod.length > 0 ? prod[0].capacity : 2;
      }

      const { results: duplicate } = await env.DB.prepare(
        "SELECT r.id FROM rooms_new r JOIN camps c ON r.camp_id = c.id WHERE c.tenant_id = ? AND r.camp_id = ? AND LOWER(r.name) = LOWER(?)"
    ...
    try {
      const roomId = path[2];
      const parsed = roomPostSchema.partial().safeParse(toSnake(await request.json()));
      if (!parsed.success) {
        return validationError(parsed);
      }
      const { camp_id, product_id, name, floor, status, bed_type, max_guests, base_price, notes, is_active } = parsed.data;

      const { results: roomCheck } = await env.DB.prepare(
        "SELECT r.id FROM rooms_new r JOIN camps c ON r.camp_id = c.id WHERE c.tenant_id = ? AND r.id = ?"
      ...
      if (name && camp_id) {
        const { results: duplicate } = await env.DB.prepare(
          "SELECT r.id FROM rooms_new r JOIN camps c ON r.camp_id = c.id WHERE c.tenant_id = ? AND r.camp_id = ? AND LOWER(r.name) = LOWER(?) AND r.id != ?"
    ...
    try {
      ...
      const { results: orders } = await env.DB.prepare("SELECT id FROM orders WHERE tenant_id = ? AND room_id = ?").bind(tenantId, roomId).all();
      if (orders.length > 0) {
...
export async function handleRatePlansRoute(request, env, tenantId) {
  const url = new URL(request.url);
  const method = request.method;
  const path = url.pathname.split('/').filter(Boolean);

  if (method === 'GET') {
    const { results } = await env.DB.prepare("SELECT * FROM rate_plans_new WHERE tenant_id = ?").bind(tenantId).all();
    return cachedJsonResponse(results);
  } else if (method === 'POST') {
    try {
      const parsed = ratePlanPostSchema.safeParse(toSnake(await request.json()));
      if (!parsed.success) {
        return validationError(parsed);
      }
    ...
    try {
      const rpid = path[2];
      const parsed = ratePlanPostSchema.partial().safeParse(toSnake(await request.json()));
      if (!parsed.success) {
        return validationError(parsed);
      }
    ...
    try {
      const rpid = path[2];
      const { results: rpInfo } = await env.DB.prepare(
      ...
      await env.DB.prepare("DELETE FROM rate_plans_new WHERE tenant_id = ? AND id = ?").bind(tenantId, rpid).run();
      return jsonResponse({ success: true });
    } catch (e) {
