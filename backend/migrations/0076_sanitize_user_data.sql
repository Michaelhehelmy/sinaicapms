...
-- Idempotent: CASE/WHEN only touches values that contain dangerous patterns.
...
-- 7. inventory_adjustments.note
UPDATE inventory_adjustments
SET note = REPLACE(REPLACE(REPLACE(
  note,
  '<script', '<scr¡pt',
  '</script>', '</scr¡pt',
  'onerror=', 'onerror​='
))
WHERE note LIKE '%<script%' OR note LIKE '%</script%' OR note LIKE '%onerror=%';
