...
- **Zone model** (`app/src/lib/routeZones.ts`): every route resolves to `marketplace` or `tenant`; `/camps /camp/*` are marketplace-only, `/book /menu /rooms` tenant-only, system prefixes (`/admin /pos /api /auth /register /login /robots.txt /sitemap.xml /404 /_astro /favicon`) never forbidden; forbidden routes render a branded 404 (ZoneGuard) — exact-path matching (siblings like `/bookings` are NOT forbidden).
