...
- **D1 `DB.batch` has NO conditional abort (2026-08-24, H1)**: a `[SELECT COUNT, INSERT]` batch is NOT atomic w.r.t. business rules — both statements commit even if the SELECT saw a conflict, so it cannot prevent double-booking. The correct race-safe pattern is an availability-guarded INSERT: `INSERT INTO orders (...) SELECT <values>, datetime('now'), datetime('now') WHERE NOT EXISTS (<overlap predicate>)` run via `DB.batch([stmt])`, then treat `result.meta.changes === 0` as the conflict signal (409). Guard bindings are appended AFTER the insert values and must mirror the read-side predicate exactly (see orders.js POST).
- **Mocked D1 statements expose no `.sql` property (2026-08-24)**: in vitest DB mocks, assert SQL text via `db.prepare.mock.calls[i][0]` (or `.map(c => c[0]).find(s => s.includes('...'))` — index-based lookups break whenever a code path conditionally adds a prepare, e.g. customer lookup by email/phone), and bind args via `stmt.bind.mock.calls[0]`.
- **Inline literals keep bind-count contracts stable (2026-08-24, H5)**: tenant resolution lookups use `(id = ? OR subdomain = ? OR custom_domain = ?) AND status = 'active'` with `status = 'active'` as an INLINE literal, not a bound param — tests (tenants-unit www-normalization) assert exact bind arrays of 3; adding a 4th bind would break them. Also: parenthesize the OR group or the `AND` binds only to the last condition and suspended tenants leak through subdomain/domain matches.
- **Strict `=== null` for D1 NULLs in guards (2026-08-24, H4)**: real D1 materializes NULL as explicit `null`, so category global-guard checks use strict `tenant_id === null`; including `=== undefined` would misfire on legacy test mocks that omit the key entirely.
...
**Scope**: 6 high-priority correctness bugs from the business logic audit (backend only — DB FROZEN, no migrations).

| Fix | File(s) | Change |
|---|---|---|
| **H1** double-booking race | `backend/src/api/orders.js` | POST /orders now uses an availability-guarded INSERT (`WHERE NOT EXISTS` overlap predicate via `DB.batch`) and returns **409 "Room no longer available"** when `meta.changes === 0`. A naive `[COUNT, INSERT]` batch would NOT work — D1 batches have no conditional abort. Guard binds `[tenantId, room_id, check_out_date, check_in_date]` mirror validateOrder exactly. |
| **H2** non-atomic tenant deletion | `backend/src/api/admin.js` | Extracted `buildTenantCascadeStmts()` (15 child tables in original FK order); single DELETE + bulk delete both run cascade + tenants-row delete in ONE `DB.batch`. Bulk response `{success, deleted: ids}` preserved. |
| **H3** plans PUT ownership gap | `backend/src/api/others.js` | PUT /plans/:id now validates NEW `camp_id` belongs to the tenant → **404 "Camp not found for this tenant"** (same message as POST). Check skipped when camp_id absent. |
| **H4** global-category mutation | `backend/src/api/categories.js` | PUT now selects `id, tenant_id`; tenant admins editing a GLOBAL category (`tenant_id === null`, strict) get **403 "Only super admins can edit global categories"**. Strict null (not undefined) so legacy mocks pass. |
| **H5** suspended tenants resolvable | `backend/src/api/tenants.js` + `middleware/tenant.js` | Both lookups: `(id = ? OR subdomain = ? OR custom_domain = ?) AND status = 'active'` — inline literal keeps the 3-bind contract; OR group parenthesized. Accepted risk noted: suspended tenants' admins lose API access via getTenant hint. |
| **H6** illegal status transitions | `backend/src/api/orders.js` | Module-level `LEGAL_TRANSITIONS` map; PATCH /orders/:id/status checks 404 → 400 → 409 `` `Illegal status transition: '<cur>' → '<new>'` ``. Unknown current state fails CLOSED. pending→[confirmed,cancelled], confirmed→[checked_in,cancelled], checked_in→[checked_out,cancelled], checked_out/cancelled terminal. |

**Tests updated/added**: orders-unit (batch mock, resequenced POST test, H1 conflict + guard-predicate tests, 6 new H6 transition tests), categories (+3 H4), others (+3 H3), admin-unit (batch mock, batch-count assertions: 16 single / 46 bulk), tenants-unit (+3 H5), tenant-middleware (+2 H5), root tenantMiddleware (SQL-shape assertion updated for parenthesized query), sse-unit (batch mock added).

**Results**: backend **37 files / 1219 passed**, root **10 files / 156 passed**, app **91 files / 1841 passed**.

**Lessons**: see new Persistent Learnings entries (DB.batch no-abort pattern, `.sql` mock property absence, inline-literal bind contracts, strict-null D1 guards).
...
## Task Log — 2026-08-23: Unified-architecture API modules (meta/tags/audit/softDelete) + camps→projects migration in camps.js

**Summary**: Landed the four unified-architecture modules plus the camps.js table migration. NEW `backend/src/utils/slug.js` (`slugify()`: trim→lowercase→hyphenate→strip→collapse; '' for unusable input). NEW `backend/src/api/meta.js` — EAV CRUD for `tenant_meta`/`project_meta` via a `createMetaRoutes(table, fkColumn, resolveEntity)` factory; routes PATCH `/reorder` (DB.batch), GET `/`, POST `/` (returns `meta.last_row_id`), PUT/DELETE `/:id` (COALESCE partial update, changes===0→404), GET `/:key`; exports `loadProjectMeta(DB, projectId)` folding rows into `{[meta_key]: meta_value}`; access model self-enforced: GETs public, writes require scope user + tenant match (super_admin bypass) — CRITICAL design point: mutation handlers call `assertWriteAccess()` UNCONDITIONALLY after entity resolution so an anonymous write can never slip through on a public mount (first draft gated on `getScope(c).user` and fell open — fixed before commit). NEW `backend/src/api/tags.js` — tagsRoutes (list requires scope.tenantId else 401; POST/PUT slugify name, UNIQUE(tenant_id,slug) precheck→409 'Tag already exists', id `'tag_'+uuid12`, DELETE = batch[junction, tag]) + projectTagsRoutes (project ownership via live projects row, POST accepts tag_ids/tag_names with find-or-create by slug, ownership-filtered INSERT…SELECT into project_tags with OR IGNORE). NEW `backend/src/api/audit.js` — best-effort `logAudit()` (own try/catch→console.error→null; serializes old/new values), GET with zod enum filters + T6 envelope where legacy `limit`/`offset` are aliases pinned to page/floor(offset/pageSize)+1, POST escape hatch defaulting tenant/user from scope. NEW `backend/src/api/softDelete.js` — `softDeleteTenant/Project`, `restoreTenant/Project` (batch; projects stamp updated_at explicitly since trg_camps_updated_at died in the 0063 rename; tenants leave it to trg_tenants_updated_at), `isSoftDeleted(row)`, and `hardDeleteTenant` = 30-statement batch purge ordered against the LIVE FK graph (orders before rooms_new [RESTRICT]; pos_recipe_ingredients/pos_transaction_items before pos_products/pos_transactions; all org-subselect children before pos_organizations/tenant_org_mapping; password_reset_tokens via admins subselect; tenants LAST) — caller must enforce super_admin. UPDATED `backend/src/api/camps.js`: every `camps` SQL ref → `projects` (+ `deleted_at IS NULL` on reads/ownership checks; rooms JOINs and subselects included); schemas gain slug/project_type(enum)/latitude/longitude/description/gallery_images(array|string union → JSON string on write, parsed back to array on read ONLY when present — list responses keep exact shape for toEqual tests); POST keeps one-camp guard FIRST (test order dependency) then derives slug from name ('' → 400 'Name must contain alphanumeric characters') with UNIQUE precheck→409 'Slug already exists'; legacy `notes` field still accepted but persisted as project_meta 'notes' via upsertNotesMeta (best-effort); PUT slug changes explicit-only w/ excluding-self uniqueness check; DELETE converted from hard cascade (5 statements) to soft delete = ownership pre-check (sole 404 source) + `UPDATE projects SET deleted_at=datetime('now'), updated_at=datetime('now')` via plain .run(); GET /:id embeds folded `meta` object (try/catch→{}); 'camp_' id prefix kept for URL/back-compat.

#### Files Changed
- `backend/src/utils/slug.js` — NEW.
- `backend/src/api/meta.js` — NEW (rewritten once to fix the fail-open mutation gate).
- `backend/src/api/tags.js` — NEW.
- `backend/src/api/audit.js` — NEW.
- `backend/src/api/softDelete.js` — NEW.
- `backend/src/api/camps.js` — projects rename + new fields + soft delete + meta embed.
- `backend/tests/products-unit.test.js` — 2 assertions → `FROM projects WHERE tenant_id = ? AND deleted_at IS NULL LIMIT 1`.
- `backend/tests/camps-unit.test.js` — 2 assertions → projects forms; DELETE test renamed 'soft-deletes camp (tombstones the row, no hard cascade)' + regression guard asserting `SET deleted_at = datetime('now')` runs and NO `DELETE FROM projects` executes.

#### Results
- Backend **1198 ✅ (37 files)** · Root integration **156 ✅ (10 files)** · All six touched/new JS files pass `node --check`. New routers NOT yet mounted in index.js (follow-up) — no route-surface change shipped.

#### Persistent Learnings (new)
- **camps/products unit mocks have NO `.batch`** and `run()` resolves `{success:true}` without `meta.changes` — handlers covered by those suites must use `.run()` + a SELECT pre-check as the sole 404 source, never `DB.batch` or `result.meta.changes`. (pos/orders mocks DO implement batch.)
- **Products POST without camp_id is asserted at exactly 3 prepare calls** (`toHaveLength(3)`): camp-resolve → tenant_org_mapping → pos_products INSERT. Never add queries to that path without updating products-unit.test.js.
- **Shared-chain mocks return the SAME queued result for every `.all()`** — handler query ORDER is load-bearing in tests (e.g. one-camp guard must precede the slug precheck or the mock's non-empty result produces the wrong 409 message).
- **Migration 0063 dropped `camps` via CREATE+COPY+DROP WITHOUT rebuilding `rooms_new.camp_id`'s FK** (still targets the dropped table) and did not recreate the camps updated_at trigger — deploy will need a follow-up migration to rebuild both.
- **Out-of-scope `camps` references remain** in `orders.js` (3), `admin.js` (5), `inbox.js` (1), `meal-schedules.js` (2), `others.js` (5), `reports.js` (3) — they will break at deploy time until migrated to `projects` (next task in the sequence).

## Task Log — 2026-08-23: Mount meta/tags/audit routers in index.js + finish camps→projects migration in remaining modules

**Summary**: Completed the two follow-ups from the previous entry. (1) MOUNTED all five routers in `backend/src/index.js`: `tenantMetaRoutes` → `/api/tenants/:tenantId/meta` and `projectMetaRoutes` → `/api/projects/:projectId/meta` (both under a method-aware `metaScope`: GET public / mutations admin), `tagsRoutes` → `/api/tags` (`tagsScope`, same model), `projectTagsRoutes` → `/api/projects/:projectId/tags` (under existing `catalogScope` — public read, admin attach/detach), `auditRoutes` → `/api/audit` (plain `resolveScope()`, no public surface). CRITICAL placement: the tenant-meta mount is registered BEFORE `app.get('/api/tenants/*', handleTenantsRoute)` because Hono runs matching handlers in REGISTRATION ORDER — the wildcard would otherwise swallow every meta request (proven empirically: meta-last → WILDCARD wins and scope never runs; meta-first → META-HANDLER wins). Used SINGLE `'/…/*'` use-lines everywhere (not the legacy bare+star dual pattern): verified that a lone `use('/x/*')` matches BOTH the exact path and every subpath with ONE middleware run, while the dual pattern double-runs resolveScope on exact-path requests (the codebase's older mounts still carry that inefficiency). Param names in mount paths must match what the routers read (`:tenantId`/`:projectId`) or params bind undefined. (2) FINISHED the camps→projects rename across the six remaining modules — 21 SQL statements total: orders.js (validate/calculate-price JOINs pure rename; availability subselect + `deleted_at IS NULL`), admin.js (stats COUNT excludes tombstones but keeps the `total_camps` wire key; cascade DELETEs/subselects pure rename so soft-deleted rows are purged too; `product_camps` junction untouched), inbox.js (historical display LEFT JOIN, no filter), meal-schedules.js (display join pure; create-guard existence check + `deleted_at IS NULL` mirroring camps.js write guards), others.js (plans list/detail JOINs + create-guard filtered; update/delete ownership subselects pure), reports.js (occupancy denominator AND numerator both filter deleted so the ratio stays consistent; bookings-by-camp historical INNER JOIN unfiltered). Verified with a temporary smoke spec against the real `app.fetch()` (9 assertions: every mount reachable, correct scope gates 401/404, tenants-wildcard not capturing meta, terminal fallback control, /api/v1 alias parity) — spec deleted after passing.

#### Files Changed
- `backend/src/index.js` — 3 imports added; tenant-meta block inserted above the Tenant routes section (with ordering rationale comment); unified meta/tags/audit block inserted after the media mount, before the API terminal fallback.
- `backend/src/api/orders.js` — 2 JOIN renames + 1 filtered availability subselect.
- `backend/src/api/admin.js` — 7 statements (stats COUNT filtered; 2× rooms cascade, 2× plans cascade, 2× DELETE renames).
- `backend/src/api/inbox.js` — 1 display-join rename.
- `backend/src/api/meal-schedules.js` — 1 join rename + 1 filtered existence check.
- `backend/src/api/others.js` — 5 statements (list/detail/create-guard filtered; 2 ownership subselects pure).
- `backend/src/api/reports.js` — 3 statements (occupancy pair filtered; by-camp join pure).
- `AGENT_LOGBOOK.md` — this entry.

#### Results
- Backend **1198 ✅ (37 files)** — unchanged from baseline, zero regressions. Smoke spec 9/9 ✅ (temporary file removed). Remaining ` camps` mentions in backend/src are comments/docstrings only; no live SQL references the dropped table anymore (supersedes the previous entry's learning about pending out-of-scope references).

#### Persistent Learnings (new)
- **Hono handler precedence is registration-order**: when two patterns match, the FIRST-registered handler answers. Route-specific mounts must be registered BEFORE catch-all wildcards for the same prefix (`/api/tenants/*` vs `/api/tenants/:id/meta`). Middleware-only matches don't preempt handlers — only earlier HANDLERS do.
- **Hono `use()` coverage on this version**: a single `use('/x/*')` matches both `/x` and `/x/deep`; a bare `use('/x')` matches ONLY `/x`. Registering both lines makes the middleware run TWICE on exact-path requests — prefer the single star line (existing dual-pattern mounts in index.js could be slimmed later).
- **Observing middleware in tests**: `c.header()` values never reach the response when a handler returns a raw `new Response(...)` (context headers apply only to `c.text()/c.json()` bodies). Use `c.set('marker', …)` read back inside handlers to prove middleware ran.
- **projectTagsRoutes contract**: `GET /api/projects/:pid/tags` resolves the project FIRST — nonexistent project ⇒ 404 `Project not found` even unauthenticated; don't assert 401 there.
- **Node cannot import src/index.js directly** (extensionless relative imports fail strict ESM resolution); wrangler/Vitest resolvers handle it. For routing smokes, write a throwaway vitest spec calling the default export's `fetch(request, env)` with a lazy DB stub (`get DB()` getter so each prepare returns fresh stubs).

## Task Log — 2026-08-23: Frontend foundation for the unified business/project schema (project-types + business-types + DynamicForm + api client)

**Summary**: Built the frontend schema layer for the unified business/project model (consumes the meta/tags/audit routers mounted backend-side earlier today). (1) NEW `app/src/lib/project-types/` — `index.ts` exports `FieldType`/`MetaFieldDef`/`ProjectTypeSchema` types, the `PROJECT_TYPES` registry, and a `getProjectType()` helper with a generic-custom fallback for unknown type strings; per-vertical schema files `camp.ts` / `supermarket.ts` / `transportation.ts` / `restaurant.ts` declare coreFields + typed metaFields (tags/textarea/number/date/select/text; select fields always carry options; `min`/`max` added as optional MetaFieldDef props for number inputs). (2) NEW `app/src/lib/business-types/index.ts` — `BusinessType`/`BusinessTypeSchema`, `BUSINESS_TYPES` registry for all six business identities (camp/supermarket/transportation/restaurant/hotel/custom), `BUSINESS_TYPE_ORDER` for deterministic pickers (custom last), `getBusinessType()` falling back to custom; contact meta built via a shared `withContact()` helper so phone/email/website labels stay consistent. (3) NEW `app/src/components/admin/DynamicForm.tsx` — controlled schema-driven form: renders core columns from a `CORE_FIELD_DEFS` catalogue (name/location/start_date/end_date/capacity/description/status) and all 8 meta widget types (tags = comma input → deduped string[]; image-gallery = one-URL-per-line textarea → string[]; json = mono textarea emitting raw text with inline 'Invalid JSON' flag that does NOT block edits); label/error/help chrome owned solely by an internal FieldShell so the Input/Select primitives never double-render error nodes; errors map lookup order `meta.<key>` → `<key>`; union-safe via `'coreFields' in schema` because BusinessTypeSchema has NO coreFields (business schemas render meta-only — this was caught by tsc and is now test-covered). (4) `app/src/lib/api.ts` — appended Project Meta / Tags / Audit sections with the exact requested signatures; wire-verified against the live routers: GET list returns raw arrays (camelCase rows `{id, projectId, metaKey, metaValue, sortOrder}`), POST bodies sent camelCase (`metaKey`/`metaValue`, `tagIds`) since handlers toSnake them, reorder PATCHes `/meta/reorder` with snake-keyed items (`sort_order` survives toSnake unchanged), audit GET forwards `entity_type`/`limit`/`offset` as query params and returns the T6 pagination envelope.

#### Files Changed
- `app/src/lib/project-types/index.ts`, `camp.ts`, `supermarket.ts`, `transportation.ts`, `restaurant.ts` — new.
- `app/src/lib/business-types/index.ts` — new.
- `app/src/components/admin/DynamicForm.tsx` — new.
- `app/src/lib/api.ts` — +3 endpoint sections (10 exported functions) after Media Upload.
- `app/tests/unit/project-types.test.ts`, `business-types.test.ts`, `DynamicForm.test.tsx`, `api-project-taxonomy.test.ts` — new (70 tests).
- `AGENT_LOGBOOK.md` — this entry.

#### Results
- Frontend **1809 ✅ (91 files)** — up from 1808 baseline with 70 new tests, zero regressions. `astro build` ✅. tsc clean on all new src files. No page/panel consumes the new modules yet (foundation only); CampsPanel/AdminApp intentionally untouched.

#### Persistent Learnings (new)
- **Controlled-input jsdom quirk**: firing `fireEvent.change` with a value equal to React's tracked controlled value is DEDUPED (no onChange). In component tests whose onChange is a bare vi.fn() (state never updates), you cannot simulate "clear the field" after typing — use a tiny stateful harness that round-trips values through useState.
- **Union prop access in shared components**: when a form must accept two schema shapes where only one carries a prop (`coreFields` on ProjectTypeSchema, absent from BusinessTypeSchema per spec), narrow with `'coreFields' in schema` instead of casting — vitest's esbuild transpile does NOT type-check, so only tsc catches it; run `tsc -p <scoped tsconfig>` on new files even when the suite is green.
- **FieldShell chrome ownership**: when wrapping ui primitives that render their own error/helper nodes (Input/Select) inside a shell that also renders chrome, NEVER pass `error`/`helperText` through to the primitive — you get duplicate visible messages and colliding DOM ids (`${id}-error`). Shell owns aria-describedby targets; primitives get aria-invalid only.
- **api.ts body-case rule of thumb**: send camelCase request bodies everywhere EXCEPT payloads whose keys are already snake_case by contract (e.g. reorder `items[].sort_order`) — backend `toSnake` normalizes camel→snake but leaves snake keys untouched, so both work; prefer matching the task-spec signature verbatim when given one.

## Task Log — 2026-08-23: Unified-schema wiring into CampsPanel (backend enum reconcile + meta persistence chain)

**Summary**: Wired the schema foundation into the admin UI end-to-end. (1) Backend `camps.js` `PROJECT_TYPES` reconciled from the stale `['camp','store','route','event','custom']` to the frontend registry vocabulary `['camp','supermarket','transportation','restaurant','custom']` (nothing pinned the old values — no DB rows, no tests; migration comments 0059 vs 0060 had drifted, frontend registry is authoritative). (2) `api.ts` +`updateProjectMeta(projectId, metaId, value)` PUT wrapper. (3) `project-types/index.ts` +serialization/diff layer: `PROJECT_TYPE_ORDER`, `encodeMetaValue`/`decodeMetaValue` (multi-value fields ↔ JSON array strings; null = delete signal because backend Zod rejects empty strings; tolerates legacy comma rows), `MetaRow`/`MetaWriteOps`, `buildMetaOps(rows, next, fields, exclude)` pure diff (creates/updates/deletes; skips excluded + foreign keys by construction so type switches never touch other schemas' data), `isMetaOpsEmpty`. (4) DynamicForm gained `fields?: 'all'|'core'|'meta'` and `excludeMetaKeys?: string[]` section filters. (5) useQueryHooks: `queryKeys.projectMeta(id)`, `useProjectMetaQuery(id?)`, `useSaveProjectMetaMutation(id)` executing a MetaWriteOps via Promise.all over set/update/deleteProjectMeta with cache invalidation. (6) CampsPanel: additive integration — Project Type select (registry-ordered), embedded `<DynamicForm fields="meta" excludeMetaKeys={['notes']}/>` below the untouched core grid, per-open seeding of ALL meta rows generically (not just active-schema fields → switching type mid-edit can't mistake an unrendered key for cleared), save chains core mutation success → buildMetaOps → metaMutation → close+refresh; submit disabled until meta query settles while editing.

#### Files Changed
- `backend/src/api/camps.js` — one-line enum reconcile (+comment).
- `app/src/lib/project-types/index.ts` — serialization + diff helpers.
- `app/src/lib/api.ts` — +updateProjectMeta.
- `app/src/components/admin/DynamicForm.tsx` — fields/excludeMetaKeys props.
- `app/src/hooks/useQueryHooks.ts` — projectMeta key + two hooks; widened pre-existing priceOverrides factory signature (`Record<string,string>` rejected optional from/to).
- `app/src/hooks/useAdminData.ts` — Camp.projectType?.
- `app/src/components/admin/CampsPanel.tsx` — type select, meta section, seeded state, chained save.
- Tests: project-types.test.ts (+15), api-project-taxonomy.test.ts (+1), DynamicForm.test.tsx (+8), CampsPanel.test.tsx (+8, mock extended with the two new hooks).

#### Results
- Frontend **1841 ✅ (91 files)** — up from 1809. Backend camps-unit **83 ✅**. Scoped tsc clean. Production build ✅. Existing pinned-behavior tests untouched (CampsPanel labels/wizard flows all still green — integration was purely additive).
- Fixed mid-task: stale-closure bug where handleSave's useCallback omitted persistMetaAndClose from deps → click handler kept an old metaValues snapshot (test caught it: meta op computed from pre-change state).

#### Persistent Learnings (new)
- **Mock TanStack mutations must resolve their options**: when a component chains `mutation.mutate(x, { onSuccess })` into follow-up work, the vi.fn() replacement MUST invoke `options.onSuccess()` or downstream assertions fail confusingly (the error surfaces on the LAST link in the chain — e.g. "onRefreshCamps not called" — not at the real break).
- **Seed EAV form state for ALL stored keys, not the active schema's**: decoding only the current schema's fields means a mid-edit type switch sees foreign keys as "cleared" (undefined → encoded null → DELETE op = data loss). Seed every row; let widgets/buildMetaOps coerce shapes.
- **useCallback dependency omission across chained callbacks**: if callback A closes over memoized callback B, B must be in A's deps even when eslint is silenced — otherwise A freezes B's first-render closure (stale state reads that tests catch only when state changes between mount and action).

## Task Log — 2026-08-24: Logic-correctness & business-operation re-audit (read-only) + live-DB verification

**Summary**: Full re-audit of backend business logic (not style): every `backend/src/api/*` module, all middleware (`sharedAuth`, `requireAuth`, `resolveScope`, `rateLimit`, `tenant`), `routes/pos/index.js` (891 lines), `index.js` route mounting/CORS/SSE, utils, and targeted migration greps — then escalated to **live production D1 introspection** via Cloudflare API when repo migrations contradicted runtime expectations. ~50 findings logged (report delivered in-session), headline items:
1. **CRITICAL — public unauthenticated order creation**: `POST /api/orders` is PUBLIC (`isOrdersPublic` includes POST in `index.js`) AND the handler trusts client-sent `total_amount` → anyone can POST bookings at any price; revenue reports ingest them.
2. **CRITICAL — deployment landmine (schema drift)**: prod `campmaster-db` `d1_migrations` stops at **0060**; migrations **0061–0066 have NEVER run against ANY database** (staging is empty). Local HEAD queries `projects` exclusively (10 API modules, zero `camps` refs) while the deployed worker still runs old `camps`-code. `deploy.sh:336` applies migrations BEFORE uploading the worker → next full deploy drops `camps` under old code (outage window; permanent breakage if worker upload fails after migrations succeed).
3. **CRITICAL — migration 0063 is destructive/incomplete**: create-copy-drop rename only re-points `project_meta`/`project_tags`; then `DROP TABLE camps` fires an implicit `DELETE FROM camps` whose ON DELETE CASCADE (declared on orders/rooms_new/plans_new/meal_schedules) **silently wipes bookings/rooms/plans/schedules**. Even if survived, `orders.camp_id` and `plans_new.camp_id` keep dangling `REFERENCES camps(id)` (0066 fixes only rooms_new + meal_schedules) → every subsequent booking INSERT fails. Chain is unvalidated anywhere.
4. HIGH cluster: non-atomic multi-statement writes without `DB.batch()` (admin tenant delete ~16 sequential DELETEs w/ missing child tables → half-deleted tenants; priceOverrides bulk upsert; tags/meta races), `softDelete.js` returns wrong batch result arm (`results[0]` vs `[1]`) for zero-project tenants, plans PUT cross-tenant write (raw `camp_id` COALESCE, no ownership check), any tenant admin can modify GLOBAL categories (`tenant_id IS NULL` branch), public tenant detail lacks status filter (suspended tenants browsable/bookable), orders TOCTOU double-booking (validate-then-insert, no unique room+dates constraint), PATCH status has no state machine (completed→cancelled allowed).

**Recalibrations (verified against migrations/live schema)**: RETRACTED cross-tenant duplicate-admin-email concerns — `admins.email` is GLOBALLY UNIQUE (0028:20), so auth.js forgot-password fallback and admin.js upsert-by-email are safe (single candidate always). DOWNGRADED free-string `role` fields — DB `CHECK(role IN ('super_admin','admin'))` fails closed; same for arbitrary `meal_category_id`/`order_state_id` (FK-enforced → generic error, no corruption). Confirmed UNIQUEs exist for subdomain/custom_domain/admins.email/order_number/orders.reference/idempotency-key/projects(tenant_id,slug).

#### Files Changed
- `AGENT_LOGBOOK.md` — this entry (audit was read-only; zero source changes).

#### Results
- Findings report delivered in-session (Executive Summary / Risk Heatmap / Immediate Actions / Follow-ups). Live verification: prod worker script contains ZERO `projects` queries + `FROM camps` present; prod `camps` table exists (1 row) with 0059/0060 columns but no projects table; staging DB empty. No tests run (no code touched).

#### Persistent Learnings (new)
- **Ground truth for schema questions = query the real D1** (`d1_migrations`, `sqlite_master.sql`, `PRAGMA table_info`) via Cloudflare API — the `migrations/` folder describes intent, not state; drift here turned a routine audit into a showstopper discovery.
